...
describe("getRandomJokes", () => {
    it("returns 3 jokes with value text by default", async () => {
        const actual = await getRandomJokes();
    ...
    it("returns 10 jokes with limiting number parameter", async () => {
        const actual = await getRandomJokes(10);
    ...
    it("returns jokes from category with provided category", async () => {
        const actual = await getRandomJokes(3, ["animal"]);
    ...
    it("returns jokes from categories starting with 'm' with provided category", async () => {
        const actual = await getRandomJokes(
            3,
            (await getAvailableCategories()).filter(it => it.startsWith("m"))
        );
...
describe("getRandomJokesData", () => {
    const target: Promise<JokeData[]> = Promise.resolve([
        {
            categories: [],
    ...
    it("returns jokes with all properties by default", () => {
        const actual = getRandomJokesData(target);

        expect(actual).toEqual(target);
    });

    it("returns jokes with matching properties with prop names", () => {
        const actual = getRandomJokesData([() => target], ["categories", "value"]);

        expect(actual).toEqual(target.map(it => ({ categories: it.categories, value: it.value })));
    });

    it("returns jokes containing matching properties only", async () => {
        const expected: JokeProperties = ["categories", "value"];
        const actual = getRandomJokesData([getRandomJoke], expected);

        expect(actual.filter(it => Object.keys(it).length == expected.length)).toHaveLength(3);
        expect(actual.filter(it => expected.every(prop => Object.keys(it).includes(prop)))).toHaveLength(3);
    });
});
